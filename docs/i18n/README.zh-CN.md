...
- 可按属性管理 Obsidian 已有的属性值候选：保留原生顺序、按混合语言名称、已确认的最近使用或笔记数排序，也可完全不提供候选而继续允许手动输入；
...
- “属性值候选”默认关闭，用于管理 Obsidian 已有的属性值候选。默认行为可保留原生顺序，也可使用名称、已确认的最近使用或笔记数排序，或设为不提供候选但继续允许手动输入。按属性候选行为使用 `属性规则 = native|name|recent|usage|none`，`*` 可对属性分组且首个匹配规则生效；置顶、置底和隐藏值仍使用 `属性规则 = 值规则`，其中隐藏规则优先；
