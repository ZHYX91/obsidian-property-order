...
- Obsidian 将目标渲染为多值列表编辑器时，以宿主属性类型为准：YAML 中的空值、`[]` 或单个受支持标量都可接收拖入值；单个标量先保留为列表元素，再按目标插入槽加入拖入值。对象、嵌套结构和多行值不得自动转换。
...
- 拖入同一 pane 内未由 Obsidian 渲染为多值列表且明确为非列表的属性时，目标显示拒绝态且不显示插入指示器；只有在该目标上松手才显示本地化 Notice，单纯经过时不提示。
