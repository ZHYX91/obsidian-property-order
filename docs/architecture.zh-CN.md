...
所有 Obsidian Properties/候选菜单选择器集中在 `src/obsidian/`。候选排序开启时，`key-suggestion-controller.ts` 通过每个 document 的 MutationObserver 收集结构和可见性变化，并在一个 animation frame 内合并增强。全 document observer 明确忽略字符内容变化；第二个 observer 只观察当前已跟踪属性名称候选菜单内部的文本变化。桌面端初始化时扫描当前 document，以兼容插件启用前已经打开的菜单；Android 启动期不执行整页首扫，只观察之后实际挂载的候选菜单，避免在 WebView 增量装载工作区时占用主线程。功能关闭时 observers 会断开；重新开启时先观察再显式扫描当前 document。只有 adapter 同时确认 Properties 上下文、支持的菜单容器以及候选项共同父节点时，controller 才能排序。

增强复用原节点，并记录原生顺序和可见性。只有候选本身及全部祖先均未被 HTML `hidden`/`aria-hidden`、插件隐藏 class 或计算样式 `display: none` / `visibility: hidden|collapse` 隐藏时，才进入可见导航顺序。键盘桥只在已增强菜单对应的属性名编辑器仍持有焦点时工作，按可见 DOM 顺序处理 ArrowUp/Down、Home/End、PageUp/PageDown 和 macOS/iOS 的 Ctrl+P/N。桥接直接维护可见 DOM 项的原生 `is-selected` class，不派发合成 mousemove，也不读取或修改 Obsidian 私有数组；Enter 直接激活当前可见 DOM 项，全部隐藏时会被阻止。
...
Obsidian 1.12.x 保留自定义 General、Value drag、Key order 三个选项卡，并提供 `tablist`/`tab`/`tabpanel`、本地化标签栏名称、`aria-selected`、roving `tabindex`、左右方向键、Home/End 和重渲染后的焦点保持。选项卡在窄宽度下保持单行横向滚动，活动标签在初次布局和 viewport resize 后自动进入可视区，纵向溢出被隐藏；桌面精细指针下最小高度为 34px，粗指针下为 44px。Obsidian 1.13+ 改用三个原生声明式设置页面，使所有简单控件进入设置搜索；这些控件覆盖默认绑定并读写 `propertyOrderSettings`，三个属性规则编辑器、“清除最近使用历史”和规则测试框则通过声明式 `render` 定义保留自定义行为。定义构造阶段不得遍历 Vault，已有属性名称只在规则编辑器实际渲染时惰性读取。清除按钮只读取 store 是否为空并执行本地删除，不触发 Vault 枚举；规则测试值只存在于当前设置 surface，既不保存也不读取属性笔记数。宽度不超过 480px 时，两条路径中的属性规则文本框、已有属性输入框与规则测试框都改为纵向占满控制区。
...
- 生产构建把 `main.js`、`manifest.json` 和 `styles.css` 直接生成到 `dist/` 顶层，使源码构建审查与本地发布检查使用同一套标准路径。`.node-version`、`package.json` 的 `engines.node`/`packageManager` 与运行时门禁共同固定 Node.js 24.19.0 和 npm 11.17.0。独立 Release workflow 以仓库级固定 concurrency group 串行化所有候选版本且不取消在途发布，并把流程隔离为只读 `verify` job 与仅标签事件可进入的写权限 `publish` job。只读 job 在运行 Node、npm 或任何仓库脚本之前用 workflow 内联逻辑核对 checkout、事件 commit、远端默认分支和标签身份；`workflow_dispatch` 还必须来自远端默认分支当前 HEAD，且候选远端标签不存在。该 job 执行完整门禁和确定性归档，并在创建标签前查询真实 Releases，要求候选同标签 Release 不存在、所有已发布的精确稳定版本都低于候选版本，且生成说明所选的最高旧版本标签是候选 commit 的祖先。标签事件若遇到已有同标签 Release，只在它是 immutable 稳定 Release、四个远端资产名称与字节精确一致、每个资产的 GitHub provenance 都精确绑定当前仓库、`.github/workflows/release.yml` signer、标签 ref 与事件 commit 且拒绝 self-hosted runner，并在核对后再次确认远端标签身份时才视为 no-op。缺失 Release 时，只读 job 把精确四资产、严格名称集合和 `SHA256SUMS` 封装成以当前 run ID/attempt 唯一命名的一次性 Actions artifact，并输出 artifact ID 与 `upload-artifact` 的裸 64 位 SHA-256。写权限 job 不 checkout、不安装依赖、不执行仓库代码；它先以内联 `git` 逻辑从远端复核标签、默认分支和可达性，再按当前 run、artifact ID、名称和 digest 下载唯一候选；REST record 可使用规范 `sha256:` 前缀，但下载字节必须重新计算裸 SHA-256 并与 action output 相同。workflow 内联且不来自仓库 checkout 的 Python 3.9+ 验证器在提取前拒绝非规范路径、目录、链接、非普通文件、加密、重复/额外/缺失项和超限 ZIP，只把精确五项以固定 basename 和 exclusive `xb` 写入私有目录；随后严格核对 ASCII/LF 四行 `SHA256SUMS`，并要求安装 ZIP 恰含固定顺序、`100644` 模式且与 loose assets 逐字节相同的三个文件。随后 workflow 才为这四个已交接附件生成构建证明并直接创建 Release。首次只读查询只把明确 `404` 视为不存在；创建后的有界核对只重试传输失败、`404`、`5xx` 和尚未达到目标状态的 `200`，其他 HTTP 状态立即失败，最终还要下载远端资产并与已交接候选逐字节比较，再逐项用同一 exact signer/repo/ref/commit digest/deny-self-hosted 策略验证远端 provenance。匹配数字版本的 active tag ruleset（禁止 update/delete 且发布 actor 无 bypass）和仓库级 immutable Releases 设置都是创建标签前必须由维护者人工核对并留证的外部前置；workflow 不持有 Administration 凭据，也不读取或修改它们。发布后的 `immutable: true` 仅验证服务器结果，不能代替发布前证据；若该外部设置缺失，服务器可能先产生公开的 mutable Release，随后 workflow 才失败。
