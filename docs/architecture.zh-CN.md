...
- 生产构建把 `main.js`、`manifest.json` 和 `styles.css` 直接生成到 `dist/` 顶层，使源码构建审查与本地发布检查使用同一套标准路径。`.node-version`、`package.json` 的 `engines.node`/`packageManager` 与运行时门禁共同固定 Node.js 24.18.0 和 npm 11.16.0。独立 Release workflow 以仓库级固定 concurrency group 串行化所有候选版本且不取消在途发布，并把流程隔离为只读 `verify` job 与仅标签事件可进入的写权限 `publish` job。两者都在运行 Node、npm 或任何仓库脚本之前用 workflow 内联逻辑核对 checkout、事件 commit、远端默认分支和标签身份；`workflow_dispatch` 还必须来自远端默认分支当前 HEAD，且候选远端标签不存在。只读 job 执行完整门禁和确定性归档，并在创建标签前查询真实 Releases，要求候选同标签 Release 不存在、所有已发布的精确稳定版本都低于候选版本，且生成说明所选的最高旧版本标签是候选 commit 的祖先。标签事件若遇到已有同标签 Release，只在它是 immutable 稳定 Release、四个远端资产名称与字节精确一致、每个资产的 GitHub provenance 都绑定当前仓库、标签 ref 与事件 commit，并在核对后再次确认远端标签身份时才视为 no-op。缺失 Release 才进入写权限 job；该 job 先重新核对身份，再锁定安装并重建三个独立文件和确定性 `property-order-<version>.zip`，随后为四个附件生成构建证明并直接创建 Release。首次只读查询只把明确 `404` 视为不存在；创建后的有界核对只重试传输失败、`404`、`5xx` 和尚未达到目标状态的 `200`，其他 HTTP 状态立即失败，最终还要下载远端资产并逐字节比较。匹配数字版本的 active tag ruleset（禁止 update/delete 且发布 actor 无 bypass）和仓库级 immutable Releases 设置都是创建标签前必须由维护者人工核对并留证的外部前置；workflow 不持有 Administration 凭据，也不读取或修改它们。发布后的 `immutable: true` 仅验证服务器结果，不能代替发布前证据；若该外部设置缺失，服务器可能先产生公开的 mutable Release，随后 workflow 才失败。
