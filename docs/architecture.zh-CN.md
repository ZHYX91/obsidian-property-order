...
- Release workflow 只响应显式 `workflow_dispatch`，默认 `verify` 只读，明确 `publish` 才可进入写权限 job；普通标签 push 不发布。只读 job 在精确版本标签上重跑完整门禁、生成确定性交接，并按当前 run 的 artifact ID 与 digest 固定上传。写 job 在首次 attestation 或 Release 变更前重新验证 `candidate.json`、便携 acceptance closure、独立 authorization、标签、commit/tree、严格资产集合和 ZIP 内外字节。四个最终资产随后都必须具备绑定本仓库、精确 workflow、标签 ref、commit 和非 self-hosted runner 的 provenance。
