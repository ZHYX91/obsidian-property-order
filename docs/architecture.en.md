...
2. Let `drop-targeting.ts` resolve a target container and insertion slot in the same pane. Obsidian's native multi-value container means the host defines the target as a list, so it remains valid even when Metadata Cache currently contains an empty value or scalar. Only a row without that container and confirmed as non-list by the snapshot becomes a rejected target. Cross-property permission is read from current settings for each event.
...
4. In `writeback.ts`, read the latest text from the same editor and validate the source/target values captured at drag start. Invoke the pure frontmatter rewrite only if leaf, file, editor identity, and content guards pass. A supported scalar in a list-typed target is safely converted into a list containing its original value, explicit null becomes an empty list, and objects or complex structures still fail closed. Commit one minimal `editor.transaction()` and verify that the editor content exactly matches the expected result. A transaction ignored or rejected by the host is a write failure.

The editor is the only text base for drag writeback, so unsaved body edits are preserved and one successful drag creates one Obsidian undo step. Obsidian always rerenders the Properties DOM from confirmed editor content; the plugin never manufactures a successful appearance by relocating pills. A missing transaction-capable editor, ineffective transaction, active leaf/file/editor change, disappearing source DOM, `pointercancel`, Escape, window blur, no-op drop, or content conflict must cancel safely rather than fall back to a non-undoable Vault write.
