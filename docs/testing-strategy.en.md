...
- grouped property-value suggestion planning with one exact-key behavior, default fallback, settings-only chip order, cross-group moves, device-local confirmed selection frequency, note-count ties, custom pinned/normal/bottom placement, exact preset identity, preset removal without note mutation, native-popup preset injection, plugin-owned no-native fallback popup, keyboard-visible order, exact property/editor context reuse, `none` manual-input preservation, lifecycle cleanup, and loss-aware legacy migration;
- schema 3-to-4, 4-to-5, and 5-to-6 settings migration, valid `recent`, grouped value-suggestion assignment normalization and loss-aware legacy migration, immediate application, persistence failure, Retry, public external-settings reload with a three-way merge and live-surface refresh, cross-instance storage serialization, rejection of saves started after unload, preservation of the cross-property preference while value drag is disabled, key-suggestion sorts and clear/rule-test controls, value default behavior, six mutually exclusive key groups, custom editor persistence, selection-frequency clearing, chooser cleanup, zero eager Vault traversal for key choices, and narrow-layout CSS;
