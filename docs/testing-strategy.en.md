...
- This desktop-plus-emulator matrix is the shared mobile release gate. Physical Android remains optional enhanced evidence whose absence is reported as unverified but never becomes a plugin-specific release prerequisite; iOS is out of scope.
...
One constant repository-wide concurrency group serializes every version without cancelling a run in progress. The Release workflow responds only to explicit `workflow_dispatch`, with `mode` defaulting to `verify`; an ordinary tag push never publishes. At the exact version tag, the read-only job runs every gate and constructs the deterministic handoff containing loose assets, the versioned ZIP, `SHA256SUMS`, and `candidate.json`, then pins its current-run artifact ID and server digest. The write-scoped job runs only for explicit `publish` and, before its first mutation, reverifies the handoff, tag/commit/tree, portable acceptance closure, independent authorization, and the SHA-256 bindings among them. Swaps, replays, private paths, field drift, and substituting another candidate digest all fail closed.
...
The workflow no longer duplicates a Python publication implementation. Candidate creation, ZIP parsing, handoff verification, publication-boundary checks, and post-verification all invoke the same locked core. A separate workflow-contract test pins triggers, exact inputs, permission separation, action commit pins, artifact ID/digest handoff, and the publish-only write job so YAML cannot bypass the core boundary.
