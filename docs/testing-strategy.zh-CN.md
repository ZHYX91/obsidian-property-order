...
- 精确 Node.js/npm 与 lockfile root 契约、发布 job 读写权限隔离、仓库代码执行前的默认分支与标签身份核对、仓库级发布串行化、真实 Release 版本与说明基线预检、existing no-op 的四资产字节与 provenance、发布后 HTTP 重试分类、三个官方附件、手动安装 ZIP 和幂等 Release 更新。
...
安装 ZIP 必须固定条目顺序、时间、权限和无关 metadata，使相同输入得到相同字节。一个仓库级固定 concurrency group 串行化所有版本且不取消在途发布。Release workflow 的 `verify` job 只有 `contents`/`attestations` 读取权限；它在 setup-node、Node、npm 和仓库脚本之前以内联逻辑核对事件、checkout、远端默认分支与标签身份，然后执行完整门禁、归档和只读发布可接受性检查。创建标签前必须从远端默认分支当前 HEAD 手工运行带候选版本的 `workflow_dispatch`；预检要求候选标签及同标签真实 Release 均不存在，所有已发布的精确稳定版本低于候选版本，并证明生成说明所选的最高旧版本标签是候选 commit 的祖先。标签事件遇到已有同标签 Release 时，还必须证明它是 immutable 稳定 Release、四资产集合与 SHA-256 精确匹配、四项 GitHub provenance 都绑定当前仓库、标签 ref 与事件 commit，并在 no-op 前复核远端标签。只有缺失 Release 才使标签专用 `publish` job 获得 `contents`/`attestations`/OIDC 写权限；该 job 在执行仓库代码前重新核对身份，再重建、核对并证明资产。首次 REST 查询只把明确 `404` 视为缺失，其他状态 fail closed；创建后的有界核对只重试传输失败、`404`、`5xx` 和尚未就绪的 `200`，确定性 `4xx` 与其他状态立即失败。仓库必须在创建标签前人工核对并留存两项外部证据：匹配数字版本标签的 active ruleset 同时限制 update/delete 且 release actor 无 bypass，repository-level immutable Releases 已启用。workflow 无 Administration 凭据，不读取或修改两项设置；发布后的 `immutable: true` 只是结果核对，不能代替前置证据。若 immutable 设置缺失，直接创建可能先留下公开 mutable Release，再由后置核对失败。推送标签前还必须确认本地工作区干净、真实宿主矩阵满足当前产品范围、CI 全绿；保护标签创建后的失败不得通过移动、删除或重建同名标签修复，应提升版本。发布后下载四个附件并核对标签 commit、版本、ZIP 目录结构和文件哈希。
