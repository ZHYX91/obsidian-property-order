...
describe("runtime contract", () => {
  it("accepts one exact Node.js and npm toolchain", () => {
    ...
    expect(parseNpmVersion("npm/11.16.0 node/v24.18.0 win32 x64")).toBe("11.16.0");
  });
  ...
  it("rejects floating or mismatched runtime declarations", () => {
    ...
    expect(() => assertRuntimeContract({
      ...
      packageJson,
    })).toThrow(/npm 11\.16\.0 is required/u);
    expect(() => parseNpmVersion("node/v24.18.0 win32 x64")).toThrow(/through npm/u);
  });
});
